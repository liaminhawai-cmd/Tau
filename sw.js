...
const CACHE = 'tau-v127';  // bump on asset changes so clients drop the old cache (v127: an open match lives at its own ?match= URL so a refresh comes back to the board instead of losing it, and a no-clock private match asks 'still playing?' every ten minutes rather than running unattended for ever -- index.html) and can be taken back (Ctrl+Z), the walkthrough teaches the rules in the order they build and freezes each slide once its goal lands, the turn clock is 60s with a no-clock option for private matches, and the sumo dohyo is sand that scuffs -- index.html, desktop/)
const ASSETS = [
