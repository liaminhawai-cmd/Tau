...
# ---------- evolution-roster.js ----------
p=Path('nn/evolution-roster.js')
s=p.read_text(encoding='utf-8')

block_re=re.compile(r"function initialiseFacePools\(s,entries\)\{.*?\n\}\nfunction reconcileFacePools\(s,entries\)\{.*?\n\}\n(?=function sync)",re.S)
if len(block_re.findall(s)) != 1:
    raise SystemExit(f'roster pool block: expected 1 match, found {len(block_re.findall(s))}')

new_block=r'''function initialiseFacePools(s,entries){
  s.facePools={};
  for(let d=1;d<=4;d++){
...
old="  if(!s.facePools)initialiseFacePools(s,entries);else reconcileFacePools(s,entries);"
new="  if(!s.facePools)initialiseFacePools(s,entries);else reconcileFacePools(s,entries);\n  compactFacePools(s,entries);"
if s.count(old)!=1: raise SystemExit(f'sync compaction hook: expected 1, found {s.count(old)}')
s=s.replace(old,new)
old="function status(dir){const s=sync(dir),faces={};for(const key of Object.keys(FACE_CAPS)){const p=s.facePools[key];faces[key]={seats:p.active.length,trial:p.trial?1:0,waiting:p.waiting.length,retired:Object.keys(p.retired).length,capacity:FACE_CAPS[key]};}"
new="function status(dir){const s=sync(dir),faces={};for(const key of Object.keys(FACE_CAPS)){const p=s.facePools[key];faces[key]={seats:p.active.length,trial:p.trial?1:0,waiting:p.waiting.length,deferred:Object.keys(p.deferred||{}).length,retired:Object.keys(p.retired).length,capacity:FACE_CAPS[key]};}"
if s.count(old)!=1: raise SystemExit(f'status deferred field: expected 1, found {s.count(old)}')
s=s.replace(old,new)
p.write_text(s,encoding='utf-8')

# ---------- elorank-legacy.js ----------
p=Path('nn/elorank-legacy.js')
s=p.read_text(encoding='utf-8')
old="const CLOSE_FLOOR = Math.min(0.9, Math.max(0, +arg('closeFloor', 0.25)));"
...
if s.count(old)!=1: raise SystemExit(f'model evidence constant: expected 1, found {s.count(old)}')
s=s.replace(old,new)

marker="const pairGamesOf = () => {"
if s.count(marker)!=1: raise SystemExit(f'pair games marker: expected 1, found {s.count(marker)}')
insert=r'''const modelNameFromFaceId = id => {
...
old="  const g = gamesOf(), pg = pairGamesOf();"
new="  const g = gamesOf(), pg = pairGamesOf(), mg = modelGamesOf();"
if s.count(old)!=1: raise SystemExit(f'pickPair evidence maps: expected 1, found {s.count(old)}')
s=s.replace(old,new)

old=r'''      // max(), not mean(): one genuinely fresh participant should be enough to make an old champion
...
new=r'''      // max(), not mean(): one genuinely fresh participant is enough to pull an old champion back in.
      ...
      const freshnessWeight = FRESHNESS_FLOOR + (1-FRESHNESS_FLOOR)*freshRaw;'''
if s.count(old)!=1: raise SystemExit(f'freshness block: expected 1, found {s.count(old)}')
s=s.replace(old,new)

old="              `freshness floor ${FRESHNESS_FLOOR}, pair floor ${PAIR_NOVELTY_FLOOR}, CI share ${CI_SHARE}`);"
new="              `freshness floor ${FRESHNESS_FLOOR}, model carry ${MODEL_EVIDENCE_SHARE}, ` +\n              `pair floor ${PAIR_NOVELTY_FLOOR}, CI share ${CI_SHARE}`);"
if s.count(old)!=1: raise SystemExit(f'scheduler log: expected 1, found {s.count(old)}')
s=s.replace(old,new)
p.write_text(s,encoding='utf-8')
