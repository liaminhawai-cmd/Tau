...
function pickPair(elo,busy){
  ...
  if(cm){const unmet=players.filter(o=>o.id!==cm.id&&((store.tsPairs||{})[canonical(cm.id,o.id)]||0)<PHYSICAL_GAMES_PER_MATCH);
    if(unmet.length){const scored=unmet.filter(o=>!busy.has(o.id)).map(o=>[pairScore(cm,o,elo,g,pair,fr),o]).filter(x=>x[0]>0);const o=weightedDraw(scored);return o?[cm,o]:'wait';}}
