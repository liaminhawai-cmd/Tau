...
const committee=process.argv.includes('--committee')?require('./committee.js').activeLeagueCommittee(dir):null;if(committee)players.push({id:committee.id,kind:'committee',spec:committee.spec,label:committee.id,depth:committee.depth||3,members:committee.members});
...
function pickPair(elo,busy){
  ...
  // The committee's sweep comes first: while it still has faces it has not met, one lane at a time
  // (it is busy while it plays) serves it one of them, drawn by the usual pair score. Its Elo is
  // the point of fielding it, and a sweep the draw could starve would never finish.
  const cm=free.find(p=>p.kind==='committee');
  if(cm){const scored=free.filter(o=>o.id!==cm.id).map(o=>[pairScore(cm,o,elo,g,pair,fr),o]).filter(x=>x[0]>0);const o=weightedDraw(scored);if(o)return[cm,o];}
...
// legitimately slow game even on a box running two trainers at once.
// Node's child_process timeout must be an integer; costMs is a measured float, so the raw
