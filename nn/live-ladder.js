...
function render(){
  if(!P)return;
  let s=Q.value.toLowerCase(),f=filterValues(),rs=P.rows.filter(r=>(!active||r.active)&&(!s||label(r).toLowerCase().includes(s))&&columnMatch(r,f)),vis=new Set(rs.map(r=>r.id));
