...
:root{color-scheme:dark;--b:#0a0d12;--p:#111720;--l:#263142;--m:#8592a6;--t:#eef4ff;--g:#f3c85c;--c:#67d5ff;--v:#b49cff}*{box-sizing:border-box}body{margin:0;background:var(--b);color:var(--t);font:14px Segoe UI,Arial;overflow:hidden}header{height:82px;display:flex;gap:14px;align-items:center;padding:12px 16px;border-bottom:1px solid var(--l)}h1{font-size:21px;margin:0}.sub{font-size:12px;color:var(--m)}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:#67e09f;margin-right:6px}.stats{display:flex;gap:7px;flex:1}.s{min-width:88px;background:var(--p);border:1px solid var(--l);padding:7px 9px;border-radius:8px}.s b{font-size:17px;display:block}.s span{font-size:9px;color:var(--m);text-transform:uppercase}input,button{background:var(--p);border:1px solid var(--l);color:var(--t);border-radius:7px;padding:7px}input{width:180px}button.on{border-color:#63738b;background:#182333}main{height:calc(100vh - 82px);display:grid;grid-template-columns:1fr 300px}.wrap{overflow:auto;border-right:1px solid var(--l)}#board{position:relative;padding-top:32px}.head,.row{display:grid;grid-template-columns:48px minmax(260px,1fr) 80px 64px 64px 180px 210px 110px;align-items:center}.head{position:sticky;top:0;height:32px;background:#0d1219;z-index:4;padding:0 10px;color:var(--m);font-size:10px;text-transform:uppercase}.head span[data-sort]{cursor:pointer;user-select:none;white-space:nowrap}.head span[data-sort]:hover{color:var(--t)}.head span.sorted{color:var(--t)}.sortmark{display:inline-block;min-width:12px;margin-left:3px;color:var(--c)}.row{position:absolute;left:6px;right:6px;height:38px;padding:0 10px;border-bottom:1px solid #1c2531;transition:transform .55s,background .3s}.row:hover{background:#151d29}.lad{background:#15130c}.anchor{box-shadow:inset 3px 0 var(--g)}.trial{box-shadow:inset 3px 0 var(--c)}.wait{opacity:.72;border-style:dashed}.old{opacity:.35}.name{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.brain{font-weight:650}.tag{font-size:10px;border:1px solid #435065;border-radius:4px;padding:2px 4px;margin-left:6px;color:#aab5c5}.dual{color:#ffd17c}.d2{color:var(--c)}.d3{color:var(--v)}.lad .brain{color:var(--g)}.elo{font-size:17px}.games,.epochs,.ci,.xrank,.state{color:#aab5c5;font-size:12px}.xrank{color:#c8a8ff}.side{padding:12px;overflow:auto}.side h2{font-size:11px;color:var(--m);text-transform:uppercase;letter-spacing:.08em}.event{background:var(--p);border:1px solid var(--l);border-radius:7px;padding:8px;margin-bottom:6px;font-size:12px}.filter-panel{position:fixed;right:12px;top:76px;z-index:30;width:min(720px,calc(100vw - 24px));display:none;grid-template-columns:repeat(4,minmax(130px,1fr));gap:8px;background:#0d1219;border:1px solid #354257;border-radius:10px;padding:10px;box-shadow:0 16px 44px #0008}.filter-panel.on{display:grid}.filter-panel label{display:flex;flex-direction:column;gap:4px;color:var(--m);font-size:9px;text-transform:uppercase;letter-spacing:.05em}.filter-panel input{width:100%;min-width:0;padding:6px;font-size:12px}.filter-actions{grid-column:1/-1;display:flex;align-items:center;justify-content:space-between;gap:10px}.hint{color:var(--m);font-size:11px;text-transform:none;letter-spacing:0}</style><header><div><h1>Tau Live Ladder</h1><div class=sub><i class=dot></i><span id=status>connecting…</span></div></div><div class=stats><div class=s><b id=games>—</b><span>rated games</span></div><div class=s><b id=inbox>—</b><span>live inbox</span></div><div class=s><b id=models>—</b><span>live faces</span></div><div class=s><b id=zeros>—</b><span>D1 / D2 zero</span></div><div class=s><b id=d3>—</b><span>D3 measured</span></div><div class=s><b id=cull>—</b><span>cull bank</span></div></div><input id=q placeholder="filter model…"><button id=cf>Column filters</button><button id=a class=on>Live faces</button><button id=all>All faces</button></header><div id=fp class=filter-panel><label>#<input data-f=place placeholder="e.g. 1..50"></label><label>brain<input data-f=brain placeholder="name / D3 / +P"></label><label>live Elo<input data-f=elo placeholder=">100"></label><label>games<input data-f=games placeholder=">=20"></label><label>epochs<input data-f=epochs placeholder="20..80"></label><label>measured 90% CI<input data-f=ci placeholder="L9 / >L10"></label><label>extrapolated rank<input data-f=xrank placeholder="xL10"></label><label>state<input data-f=state placeholder="seat / retired"></label><div class=filter-actions><span class=hint>Numbers: exact, &gt;100, &lt;50, &gt;=20, or 20..80. Text fields match anywhere.</span><button id=clearf>Clear column filters</button></div></div><main><div class=wrap><div id=board><div class=head><span data-sort=place>#<i class=sortmark></i></span><span data-sort=brain>brain<i class=sortmark></i></span><span data-sort=elo>live Elo<i class=sortmark></i></span><span data-sort=games>games<i class=sortmark></i></span><span data-sort=epochs>epochs<i class=sortmark></i></span><span data-sort=ci>measured 90% CI<i class=sortmark></i></span><span data-sort=xrank>extrapolated rank / 90% CI<i class=sortmark></i></span><span data-sort=state>state<i class=sortmark></i></span></div></div></div><aside class=side><h2>Readout</h2><div id=read class=event>Waiting…</div><h2>Incoming results</h2><div id=recent></div></aside></main><script>
const B=document.getElementById('board'),Q=document.getElementById('q'),A=document.getElementById('a'),ALL=document.getElementById('all'),CF=document.getElementById('cf'),FP=document.getElementById('fp'),CLEARF=document.getElementById('clearf'),FINS=[...FP.querySelectorAll('input[data-f]')],HEADS=[...document.querySelectorAll('.head span[data-sort]')];
let P=null,active=true,nodes=new Map,sortKey=null,sortDir=1;
A.onclick=()=>{active=true;A.className='on';ALL.className='';render()};
ALL.onclick=()=>{active=false;ALL.className='on';A.className='';render()};
Q.oninput=render;
CF.onclick=()=>{let on=!FP.classList.contains('on');FP.classList.toggle('on',on);CF.classList.toggle('on',on)};
CLEARF.onclick=()=>{FINS.forEach(x=>x.value='');render()};
FINS.forEach(x=>x.oninput=render);
HEADS.forEach(h=>h.onclick=()=>{let k=h.dataset.sort;if(sortKey===k)sortDir*=-1;else{sortKey=k;sortDir=1}render()});
document.addEventListener('keydown',e=>{if(e.key==='Escape'){FP.classList.remove('on');CF.classList.remove('on')}});
...
function ciText(r){return r.rankLo!=null&&r.rankHi!=null?ciBound(r.rankLo,r.rankLoEdge)+' – '+ciBound(r.rankHi,r.rankHiEdge):'—'}
function xrankText(r){return Number.isFinite(r.extrapRank)?'xL'+r.extrapRank.toFixed(1)+(Number.isFinite(r.extrapRankLo)&&Number.isFinite(r.extrapRankHi)?' [xL'+r.extrapRankLo.toFixed(1)+' – xL'+r.extrapRankHi.toFixed(1)+']':''):'—'}
function numMatch(v,q){
  q=String(q||'').trim();if(!q)return true;if(!Number.isFinite(v))return false;
  let z=q.indexOf('..');if(z>=0){let a=Number(q.slice(0,z).trim()),b=Number(q.slice(z+2).trim());return Number.isFinite(a)&&Number.isFinite(b)&&v>=Math.min(a,b)&&v<=Math.max(a,b)}
  for(let op of ['>=','<=','>','<','='])if(q.startsWith(op)){let n=Number(q.slice(op.length).trim());if(!Number.isFinite(n))return false;return op==='>='?v>=n:op==='<='?v<=n:op==='>'?v>n:op==='<'?v<n:v===n}
  let n=Number(q);return Number.isFinite(n)&&v===n
}
function textMatch(v,q){q=String(q||'').trim().toLowerCase();return !q||String(v??'').toLowerCase().includes(q)}
function filterValues(){let o={};FINS.forEach(x=>o[x.dataset.f]=x.value);return o}
function columnMatch(r,f){return numMatch(r.place,f.place)&&textMatch(label(r),f.brain)&&numMatch(r.elo,f.elo)&&numMatch(r.games,f.games)&&numMatch(r.epochs,f.epochs)&&textMatch(ciText(r),f.ci)&&textMatch(xrankText(r),f.xrank)&&textMatch(r.phase||'historical',f.state)}
function sortValue(r,k){
  if(k==='place')return r.place;
  if(k==='brain')return label(r).toLowerCase();
  if(k==='elo')return r.elo;
  if(k==='games')return r.games;
  if(k==='epochs')return r.epochs;
  if(k==='ci')return Number.isFinite(r.rankLo)?r.rankLo:null;
  if(k==='xrank')return Number.isFinite(r.extrapRank)?r.extrapRank:null;
  if(k==='state')return String(r.phase||'historical').toLowerCase();
  return null
}
function compareRows(a,b){
  let av=sortValue(a,sortKey),bv=sortValue(b,sortKey),am=av==null||(typeof av==='number'&&!Number.isFinite(av)),bm=bv==null||(typeof bv==='number'&&!Number.isFinite(bv));
  if(am!==bm)return am?1:-1;if(am&&bm)return a.id.localeCompare(b.id,undefined,{numeric:true});
  let c=typeof av==='number'&&typeof bv==='number'?av-bv:String(av).localeCompare(String(bv),undefined,{numeric:true,sensitivity:'base'});
  return c?c*sortDir:a.id.localeCompare(b.id,undefined,{numeric:true})
}
function updateSortMarks(){HEADS.forEach(h=>{let on=h.dataset.sort===sortKey;h.classList.toggle('sorted',on);h.querySelector('.sortmark').textContent=on?(sortDir>0?'▲':'▼'):''})}
function render(){
  if(!P)return;
  let s=Q.value.toLowerCase(),f=filterValues(),rs=P.rows.filter(r=>(!active||r.active)&&(!s||label(r).toLowerCase().includes(s))&&columnMatch(r,f));
  if(sortKey)rs=rs.slice().sort(compareRows);updateSortMarks();
  let vis=new Set(rs.map(r=>r.id));
  ...
  rs.forEach((r,i)=>{
    ...
    e.querySelector('.name').innerHTML=n;e.querySelector('.elo').textContent=Number.isFinite(r.elo)?Math.round(r.elo):'—';e.querySelector('.games').textContent=Math.round(r.games);e.querySelector('.epochs').textContent=Number.isFinite(r.epochs)?Math.round(r.epochs):'—';e.querySelector('.ci').textContent=ciText(r);e.querySelector('.xrank').textContent=xrankText(r);e.querySelector('.state').textContent=r.phase||'historical'
  });
