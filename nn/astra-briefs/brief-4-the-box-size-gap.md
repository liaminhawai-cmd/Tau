...
Over the substeps where every pose *is* in contact, the smallest such minimum is **4.7e-4u to 9.6e-3u** across the five boxes, always at the first qualifying substep. Two cautions on that figure. It is a minimum over a 200-pose sample, so it can only go down with more poses: the throw-bound thread's own run at the same box gets 9.2e-4u where ours gets 6.5e-3u, which is sampling, not disagreement. And it is a different quantity from the smallest gain any single pose ever has, which is 2.8e-7u to 1.1e-4u and which nothing ever sums. Order 1e-3u a substep near the start, rising to 0.059u through the dwell, is the honest shape.

**No pose's contact ever lapses once it has begun**, at any box size tried, so "in contact" is a single interval per pose. That matters for the shape of the obligation below. That matters because `throw-theorem.md` already found that uniform bounds over the sweep give a total of only 0.44u by 28 degrees, dominated by the early substeps where the normal is still 68 degrees off radial, and that per-slab bounds would follow the engine's real 3.18u closely. Per-slab is precisely what this measurement says is available.
...
- Attached: `throw-cert-at-1cf90f63f.js`, the checker as it stands; `THROW-CONTACT-LEMMAS-at-1cf90f63f.md`, the write-up it implements, whose sections 8 to 10 cover the park work, the second arm and the gap; and the scripts behind section 3, `park-monotone.js`, `sustained-contact.js` and `gain-floor.js`, with their outputs.

  The write-up is pinned at the commit whose certificate results this brief quotes. Its author has since added a section 11 writing up his own version of the section 3 measurement, which this snapshot therefore predates. Nothing in it contradicts anything here — it is the independent run of the same question, and it agrees on every headline — but if you want that section, ask and it can be sent.
