...
Same conventions as briefs 2 and 3: board units `u`, degrees unless a formula says radians. Everything below was measured today against the engine's own code. Where a number is the checker's rather than the engine's, it says so. The checker's current source and the write-up it implements are attached, both at commit `1ba97f2a9` and both verified against the repository by blob hash.
...
- **(a) Does a slab map into itself?** Thin along the contact normal, wide across it. This replaces (H4) over a box, and with (b) answered it is the one hypothesis left: what has to hold is invariance in the two *tangential* directions, since the normal direction is handled below. The thinness is not an assumption about where the poses happen to be; it is what the contact shell does to them, and section 9's shell argument already pins the distance from both sides. Is the slab the right object, is there a better one, and what is the shape of a proof that the push maps such a set into itself?
  ...
  We can say what such a proof has to deliver, because the sideways half has now been measured too. Splitting each pose's deviation at `G = (hf n_x, hf n_y, hf rn)`, the gradient of the pair's distance, the along-`G` part collapses onto the shell within three substeps (0.152u, 0.095u, 0.038u, 0.00017u at `+-0.1u`) and stays there, which is (b)'s reset seen from the other side. Across `G`, from first contact to the centre's throw, the spread goes 0.02171 to 0.02427u at `+-0.0125u` and 0.17366 to 0.17883u at `+-0.1u`: a factor of 1.03 at the three larger boxes, with the end spread exactly proportional to the box and the factor agreeing to three decimals. A map whose output scales with its input is **linear in the box size**, which is the signature of no threshold, since the enclosure's growth was nonlinear and that is exactly what gave it one. The shape of the growth is a small sustained rate after an initial dip, not a transient: at `+-0.1u` the spread contracts at 0.9996 a substep from first contact to about substep 35, falling to 0.16404u, then grows at 1.0017 rising to 1.0025 a substep for the remaining forty-odd. The window nets 1.03 only because the dip pays for most of the rise. The worst single substep is still not a figure to compound over the whole sweep, since it is reached only at the end. A tube 1.05x the starting box's tangential extent covers every case tried, and 1.2x covers it with room.

  So (a) does not need a contraction: the set does spread. It needs a per-substep tangential factor bounded by about 1.003, holding over a set that is thin along `G`. Two cautions we would rather state than have found. The split is relative to a direction that itself turns as the contact walks down the leg, so some of the late growth is `G` rotating and relabelling a deviation rather than the set spreading — an upper bound on the spreading, which is the useful direction, but the park's factor should not be attributed to the dynamics without separating the two. And the `+-0.0125u` row does not fit the proportionality: its end spread is about 0.002u wider than the line, reproducing exactly at 60, 200 and 500 poses, so it is deterministic and unexplained. Measured independently here, the profile reproduces: contraction at 0.99975 a substep to a trough at substep 32, then growth at 1.0007 rising to 1.0016, and the same anomaly in the small boxes. We measured the whole deviation rather than splitting it at `G`, and the two turn out to be the same number to five decimals, because once the along-`G` part has collapsed to 2e-4u it adds in quadrature to a tangential part three orders larger and vanishes. The split is therefore free.
...
- Attached: `throw-cert-at-1ba97f2a9.js`, the enclosure checker as it stands; `THROW-CONTACT-LEMMAS-at-1ba97f2a9.md`, the write-up it implements, whose sections 8 to 10 cover the park work, the second arm and the gap, section 13 the rigorous H3 bound and the slab result, section 14 the thickness argument of question 1(b) and section 15 the tangential measurement under 1(a); `gain-bound-at-1ba97f2a9.js`, `slab-shape-at-1ba97f2a9.js`, `thickness-at-1ba97f2a9.js` and `tangential-at-1ba97f2a9.js` with their outputs, the four programs behind those sections; and the scripts behind section 3, `park-monotone.js`, `sustained-contact.js`, `gain-floor.js`, `slab-start.js`, `lapse.js`, `gap-spread.js`, `thickness-spread.js` and `tangential-spread.js`, with their outputs.
