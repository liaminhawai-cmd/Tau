...
> **Sent, answered, and superseded in part on the throw side (see `brief-3-the-vertex-dwell.md`).** Two statements in section 1 turned out to be wrong, and are left here as written rather than edited, since this is the text that was sent. First, "shrinking the starting box does not help" is false for the first crossing: at a +-0.002u box the checker passes it and reaches 24.33 degrees. Second, the third item in the vertex table is not a crossing; the victim's contact point arrives at its phi = 30 vertex at substep 74, sweep 24.67, and sits exactly on it for the next 25 substeps, through the throw at 28.00 and fourteen substeps past it. (The angles in brief 2's vertex table are also off by about 0.18 degrees: they came from a trace that divided the sweep into 139 substeps instead of the search's 138. See brief 3.) Job 2, the oriented split, is unaffected.
