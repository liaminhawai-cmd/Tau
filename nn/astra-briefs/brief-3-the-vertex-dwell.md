...
> **Sent and answered on 2026-09-18. One statement in question 3 is wrong and is left as written, since this is the text that was sent.** The park is not the same interval before and after the push. Measured from the pose *entering* each substep it runs 74 to 98; measured from the pose *leaving* it, after the push, it runs 79 to 104. Both are twenty-five substeps, offset by five. So at substeps 74 to 78 a pose enters parked and leaves unparked, and at 99 to 104 the reverse, and a certificate asserting that the park persists from substep 74 across its first push is false at the centre trajectory, as Astra said. What is true, and is the useful version, is that 79 to 98 is parked both entering and leaving: twenty substeps, stable across the push, containing the throw at 84. Nothing else in the brief depends on it; the measurement convention it trips over is the one stated in section 1.
...
**So the run ends at the combined event.** The checker's failure is at substep 73, 24.33 degrees. That substep is the attacker's own `phi = 15` chord-vertex crossing, and it is also the substep in which the victim's contact arrives at its `phi = 30` vertex, 0.005u from it; the victim is exactly on it from 74. So the run did not die before the park or inside it, it died at the substep where both events happen. The remaining 3.67 degrees to the throw is that combined event followed by a park that covers the whole rest of the way.

    That is where the checker stood when this brief was started, and it is the shape of the problem. Since then the park has been made exact and the regimes kept separate, and the run goes through substep 73 and dies at 81, three substeps short of the throw. Section 2 says what that changed and what it did not.
...
During a park the victim's contact point is not something to be localised at all. It is a **known material point of the victim**, the vertex at arc angle 30 degrees on leg 0, whose position is an exact function of the pose,

    p(q) = (x, y) + R sin(30 deg) (cos(rot), sin(rot)),   at height R cos(30 deg),

so the lever arm `r = p_xy - c` is exact, with no interval at all. On the attacker's side the closest point is interior to a chord, so it is the foot of the perpendicular from a known point onto a fixed segment, which is again exact for a fixed attacker pose. That much is built and it works: making both sides of a park exact, and collapsing the two chord pairs that report a minimiser at the shared vertex into one regime, took the checker through the combined event at substep 73 as a single regime, to substep 81 of the 84 where the throw lands.

I first drew the obvious conclusion from that — a park has fewer unknowns, so it must be the easy regime — and it is wrong, which is worth stating because it is the kind of wrong that looks right. **Knowing the contact point exactly does not buy a tight normal.** What made the interior normal tight was never the contact point; it was perpendicularity to *both* chords, which makes the normal parallel to `uA x uV`, a function of the two chord *directions* alone. The attacker's chord is fixed within a substep and the victim's turns only by the set's rotation span, so the interior cone is **insensitive to translation**: it does not care how far apart the poses in the set are, only how differently they are turned.

A park is exactly where that fails. The normal is perpendicular to the attacker's chord only, so its direction is the projection of `vertex - A0` onto the chord's normal plane, and it swings as the victim **translates**. Measured here, over 120 poses from a +-0.0005u / +-0.002-degree box, with the true hub spread flat at about 0.0012u for the whole sweep:

| substep | regime | true normal cone |
|---|---|---|
| 60 | interior | 0.0005 deg |
| 72 | interior | 0.0004 deg |
| 74 | park | 0.0116 deg |
| 84 | park (the throw) | 0.0136 deg |
| 96 | park | 0.0182 deg |

Twenty-five times wider, at an unchanged pose spread, the moment the park begins. So the park has fewer unknowns *and* a more sensitive normal, and the two facts do not cancel.

The questions, in the order I care about them:

1. **Does a simultaneous two-leg vertex event need different treatment from a single one?** Since the first draft of this brief, the answer looks like "no, but you have to stop merging the regimes": keeping the contact regimes as genuinely separate sets, rather than hulling them into one enclosure at the end of each substep, took the checker through substep 73 as a single regime with a single chord pair. That is evidence for "two independent branchings that happen to coincide". It is worth your eye anyway, because the reasoning below says it should have been harder than that, and we would rather know why it was not than assume we got lucky. At substep 32 one leg has a vertex event and the other leg's contact is 1.42u of arc from its nearest vertex: one thing happening, in isolation, and a small box gets through it. At substep 73 both legs have one in the same substep. Are those just two independent branchings that happen to coincide, so that a scheme handling each separately handles both, or does the pair need something the single case does not?
...
2. **What replaces Lemma 2's translation-insensitivity during a park?** This is the live one. The park contact *is* exactly determined, and that formula is written and in the code, so the question is not "what is it". It is that the exact formula has a derivative with respect to translation that the interior formula does not, and the set has to be carried through the park somehow. One thing is known and does not seem to be enough: the post-push constraint puts the victim's vertex on a cylinder of radius D about the attacker's chord, which removes one dimension of its freedom but not the angular one that sets the normal. Is there a second exact relation during a park that pins the angular position too, or is the honest answer that a park has to be carried in different coordinates — say, the vertex's cylindrical position about the attacker's chord, in which the normal is a coordinate rather than a derived quantity?

    For scale: the true pose spread is flat at 0.0023u in foot displacement for the entire sweep, park included, and the true park cone is under 0.02 degrees. The checker's enclosure there is 0.075u and its park cone 3.6 degrees. So the park is not intrinsically expensive; the enclosure is loose, and the park's translation-sensitivity is what turns that looseness into a runaway where interior contact absorbed it.
...
- Reach, in substeps of the 138: the checker stopped at 73 at a +-0.001u box and at 34 at +-0.005u; with the park made exact and the regimes carried separately it reaches 81, at +-0.0002u. The throw is at 84. The two-substep cost of the sound projection bound is included throughout.
