...
function main(){
  ...
  fs.mkdirSync(medalDir,{recursive:true});const meta={updated:new Date().toISOString(),machine,basis:'global 90% Elo lower bound across D1-D4',medals:{}},worker=JSON.parse(JSON.stringify(sum)),paths=[];
  for(let i=0;i<3;i++){const r=ranked[i],name=names[i],dst=path.join(medalDir,name+'.json');copy(r.src,dst);meta.medals[name]={source:r.model,sourceId:r.id,depth:r.depth,games:r.games,elo:r.elo,eloLo:r.eloLo,eloHi:r.eloHi};
    worker.players[`medal-${name}@D${r.depth}`]={...r,model:path.join('nn','medals',machine,name+'.json'),medal:name,machine,sourceId:r.id};delete worker.players[`medal-${name}@D${r.depth}`].src;paths.push(path.relative(root,dst).replace(/\\/g,'/'));}
  ...
  log(`[${machine}] `+names.map(n=>`${n}=${meta.medals[n].source}@D${meta.medals[n].depth} EloLo${Math.round(meta.medals[n].eloLo)}`).join(' | '));publish(paths,machine);
}
