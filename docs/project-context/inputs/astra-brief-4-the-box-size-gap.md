...
| box, per axis | thrown | poses with a non-increasing step, whole sweep | smallest step over substeps 74-98 | smallest step over the whole contact window | box minimum clears the rim at |
|---|---|---|---|---|---|
| `+-0.1u / +-0.01 rad` | 200/200 | 0 | 0.0836u | — | substep 85 |
| `+-0.125u / +-0.01 rad` | 200/200 | 0 | 0.0628u | 4.0e-5u at substep 14 | substep 85 |
| `+-0.125u / +-0.05 rad` | 200/200 | 0 | 0.0616u | — | substep 89 |
| `+-0.25u / +-0.01 rad` | 200/200 | 0 | 0.0605u | 1.1e-4u at substep 11 | substep 86 |
| `+-0.5u / +-0.01 rad` | 200/200 | 0 | 0.0594u | 8.6e-5u at substep 6 | substep 87 |
...
**Through the dwell the increments are enormous relative to the widths in play, and they barely move with the box.** The smallest across 1000 park trajectories is 0.059u, against enclosure widths of 0.01 to 0.1u, and it falls only from 0.0628u to 0.0594u as the box grows fourfold. Through the dwell the gain runs 0.05 to 0.09u a substep.

**Over the whole contact window the picture is quite different, and this is the number that decides whether (H3) is provable.** The minimum gain over a pose's contact window is 4.0e-5u at substep 14 at a `+-0.125u` box, 1.1e-4u at substep 11 at `+-0.25u`. Both sit at a grazing first contact, not in the dwell, and they are three orders of magnitude below the comfortable park figure. (The exact value depends on where a pose's contact is deemed to start: taking each pose's first substep with a nonzero gain, as here, catches grazings that a contact test with a threshold excludes, and the throw-bound thread's own probe puts the minimum at 9e-4 to 6e-3u instead. The agreement that matters is on the shape: the minimum is early and grazing, never in the dwell.) That matters because `throw-theorem.md` already found that uniform bounds over the sweep give a total of only 0.44u by 28 degrees, dominated by the early substeps where the normal is still 68 degrees off radial, and that per-slab bounds would follow the engine's real 3.18u closely. Per-slab is precisely what this measurement says is available.
