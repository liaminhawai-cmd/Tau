...
def evaluate(domain):
    ...
    loG=math.inf;hiG=-math.inf;loA=math.inf;hiA=-math.inf;loC=math.inf;hiC=-math.inf;rnMax=0;count=0;regimes=set()
    for ai in range(12):
      for bi in range(12):
    ...
    return dict(g=[loG,hiG],horizontalAdvancePerRadian=[loA,hiA],closingPerRadian=[loC,hiC],rnMax=rnMax,regimes=sorted(regimes),branchCount=count)

if __name__=='__main__':
    import argparse,time
    ap=argparse.ArgumentParser();ap.add_argument('--n',type=int,default=4);ap.add_argument('--step',type=int,default=84);args=ap.parse_args();start=__import__('time').time()
    h=.125;hr=.01
    REF=REFERENCES[args.step-1]['pre']
    REFCONTACT=REFERENCES[args.step-1]['closest']
    raw=[V(down(REF['x']-h),up(REF['x']+h)),V(down(REF['y']-h),up(REF['y']+h)),V(down(REF['rot']-hr),up(REF['rot']+hr)),V((DEG*(args.step-1)/3).lo,(DEG*args.step/3).hi)]
    cells=[]
    for x in raw:
      ends=[x.lo+(x.hi-x.lo)*i/args.n for i in range(args.n+1)];ends[0]=x.lo;ends[-1]=x.hi
      cells.append([V(ends[i],ends[i+1]) for i in range(args.n)])
    results=[]
    for cell in itertools.product(*cells):results.append(evaluate(cell))
    out={'referencePose':REF,'poseHalfWidths':[h,h,hr],'sweepDegrees':[(args.step-1)/3,args.step/3],
      'domain':[x.bounds() for x in raw],'subboxes':len(results),'g':[min(r['g'][0] for r in results),max(r['g'][1] for r in results)],
      'horizontalAdvancePerRadian':[min(r['horizontalAdvancePerRadian'][0] for r in results),max(r['horizontalAdvancePerRadian'][1] for r in results)],
      'closingPerRadian':[min(r['closingPerRadian'][0] for r in results),max(r['closingPerRadian'][1] for r in results)],
      'rnMax':max(r['rnMax'] for r in results),'branchCount':sum(r['branchCount'] for r in results),'regimes':sorted(set(tuple(s) for r in results for s in r['regimes'])),
      'seconds':time.time()-start,'scope':'Closest pair between attacker leg 0 and victim leg 0, all endpoint/interior regimes; real geometry; no trajectory containment or other-contact certificate.'}
    (HERE/f'slab-bounds-step{args.step}.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
