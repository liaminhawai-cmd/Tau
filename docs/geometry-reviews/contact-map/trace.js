...
for(let k=1;k<=tr.length;k++){
 ...
 for(const p of row.pushes){
  ...
  rows.push({k,iter:p.iter,kind:p.kind,i:p.i,j:p.j,gap,hf:p.hf,rn:p.rn,grad2:s,lambda:p.lambda,residual,identityError,a:best.a,b:best.b,s:best.s,t:best.t,before:p.before,att:row.att});
 }
}
